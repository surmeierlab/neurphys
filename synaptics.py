...
def analyze_current(df, bsl_start, bsl_end, start_time, end_time, sign="min",
    ...
    tau_plot: boolean, default = False
        return x, y, and fit_y values for fit associated with the tau calculation
    ...
    ***Note that if tau_plot is True, weighted tau will be returned even if 
    ...
    if tau_plot:
        ...
        tau, x_vals, y_vals, fit_vals = util.calc_decay(df_bsl, peak, peak_time,
                                                           tau_plot)
...
def calc_ppr(df, bsl_start, bsl_end, start_time, end_time, stim_interval,
    ...
    #nudge first end slightly to the left so that the two regions don't overlap
    peak1 = find_peak(df_bsl, start_time, first_end-0.0001,
                      sign=sign)["Peak"].values
    peak2 = find_peak(df_bsl, first_end, end_time, sign=sign)["Peak"].values
